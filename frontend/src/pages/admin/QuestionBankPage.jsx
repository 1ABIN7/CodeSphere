...
const EMPTY_FORM = {
  ...
  tags: '', options: '', correctAnswers: '', codingProblemId: '', sqlSetup: '', sqlTestCases: [{ name: 'Sample dataset', setupSql: '', expectedRows: '[]', hidden: false }], apiTestCases: '[{\n  "name": "GET root",\n  "method": "GET",\n  "path": "/",\n  "expectedStatus": 200,\n  "expectedBody": "{}",\n  "hidden": false\n}]', passageText: '', readingDurationSeconds: 0, minWordCount: 0, maxWordCount: 2000, points: 1, negativeScore: 0, subQuestions: [],
};
...
export default function QuestionBankPage() {
  ...
  const chooseQuestionType = (questionType) => setForm({ ...form, questionType });
  ...
  const openEdit = async (question) => {
    ...
    setForm({
      ...
      apiTestCases: question.apiTestCases || '[]',
  ...
  const save = async (event) => {
    ...
    let apiTestCases = null;
    try { apiTestCases = form.questionType === 'API_IMPLEMENTATION' ? JSON.stringify(JSON.parse(form.apiTestCases)) : null; }
    catch { setSaving(false); toast.error('HTTP test cases must be valid JSON.'); return; }
  ...
  return <div className="container fade-in">
    <div className="page-header" style={{ display: 'flex', justifyContent: 'space-between', gap: 16 }}>
      ...
      <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}><Link className="btn btn-secondary btn-sm" to="/admin/problems">Task Center</Link><button className="btn btn-secondary btn-sm" onClick={() => exportQuestions('csv')}>Export CSV</button><label className="btn btn-secondary btn-sm" style={{ cursor: 'pointer' }}>{importing ? 'Importing...' : 'Import'}<input type="file" accept=".csv,.json" onChange={importQuestions} style={{ display: 'none' }} disabled={importing} /></label><button className="btn btn-primary" onClick={openCreate}>+ Create question</button></div>
    </div>
    <div className="card">
    ...
    {editorOpen && <div className="modal-backdrop" role="dialog" aria-modal="true"><form className="card question-editor" onSubmit={save} style={{ width: 'min(800px, 94vw)', maxHeight: '90vh', overflowY: 'auto', padding: 28 }}>
      ...
      {form.questionType === 'API_IMPLEMENTATION' && <div style={{ marginTop: 14 }}><div className="text-secondary">Candidate code must be a JavaScript Node HTTP server listening on port 3000. Every request runs inside the same network-disabled container as that server.</div><label className="label" style={{ display: 'block', marginTop: 10 }}>HTTP test cases (JSON)<textarea className="textarea" required value={form.apiTestCases} onChange={(event) => setForm({ ...form, apiTestCases: event.target.value })} placeholder={'[{"name":"GET root","method":"GET","path":"/","expectedStatus":200,"expectedBody":"{\\"ok\\":true}","hidden":false}]'} /><small className="text-secondary">Each case accepts name, method, path, body, expectedStatus, expectedBody, and hidden.</small></label></div>}
