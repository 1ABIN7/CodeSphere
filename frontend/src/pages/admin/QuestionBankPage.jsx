...
export default function QuestionBankPage() {
  ...
  const openEdit = (question) => {
    ...
    setForm({
      ...
      subQuestions: (question.subQuestions ?? []).map((subQuestion) => ({ title: subQuestion.title ?? '', content: subQuestion.content ?? '', options: Array.isArray(subQuestion.options) ? subQuestion.options.join('\n') : '', correctAnswers: subQuestion.correctAnswers ?? '', points: subQuestion.points ?? 1 })),
    });
    setEditorOpen(true);
  };
  ...
  return <div className="container fade-in">
    ...
    {editorOpen && <div className="modal-backdrop" role="dialog" aria-modal="true"><form className="card" onSubmit={save} style={{ width: 'min(720px, 94vw)', maxHeight: '90vh', overflowY: 'auto', padding: 24 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12, marginTop: 14 }}><label className="label">Type<select className="select" value={form.questionType} onChange={(event) => setForm({ ...form, questionType: event.target.value })}><option value="MCQ_SINGLE">Multiple choice</option><option value="MCQ_MULTI">Multiple select</option><option value="WRITTEN">Written</option><option value="CODING">Coding</option><option value="READING_COMPREHENSION">Reading comprehension</option><option value="FILE_UPLOAD">File upload</option></select></label><label className="label">Difficulty<select className="select" value={form.difficulty} onChange={(event) => setForm({ ...form, difficulty: event.target.value })}><option value="EASY">Easy</option><option value="MEDIUM">Medium</option><option value="HARD">Hard</option></select></label><label className="label">Category<input className="input" value={form.category} onChange={(event) => setForm({ ...form, category: event.target.value })} /></label></div>
      ...
      {form.questionType === 'CODING' && <label className="label" style={{ display: 'block', marginTop: 14 }}>Coding problem ID<input className="input" required type="number" min="1" value={form.codingProblemId} onChange={(event) => setForm({ ...form, codingProblemId: event.target.value })} placeholder="Existing problem ID" /></label>}
      ...
      {form.questionType === 'READING_COMPREHENSION' && <div style={{ marginTop: 14 }}><div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}><label className="label">Passage questions</label><button className="btn btn-secondary btn-sm" type="button" onClick={() => setForm({ ...form, subQuestions: [...form.subQuestions, { title: `Question ${form.subQuestions.length + 1}`, content: '', options: '', correctAnswers: '', points: 1 }] })}>+ Add question</button></div>{form.subQuestions.map((subQuestion, index) => <div className="card" key={index} style={{ padding: 14, marginTop: 10 }}><div style={{ display: 'flex', justifyContent: 'space-between', gap: 12 }}><strong>Passage question {index + 1}</strong><button className="btn btn-ghost btn-sm" type="button" onClick={() => setForm({ ...form, subQuestions: form.subQuestions.filter((_, itemIndex) => itemIndex !== index) })}>Remove</button></div><label className="label" style={{ display: 'block', marginTop: 8 }}>Title<input className="input" value={subQuestion.title} onChange={(event) => setForm({ ...form, subQuestions: form.subQuestions.map((item, itemIndex) => itemIndex === index ? { ...item, title: event.target.value } : item) })} /></label><label className="label" style={{ display: 'block', marginTop: 8 }}>Question<textarea className="textarea" value={subQuestion.content} onChange={(event) => setForm({ ...form, subQuestions: form.subQuestions.map((item, itemIndex) => itemIndex === index ? { ...item, content: event.target.value } : item) })} /></label><label className="label" style={{ display: 'block', marginTop: 8 }}>Choices (one per line)<textarea className="textarea" value={subQuestion.options} onChange={(event) => setForm({ ...form, subQuestions: form.subQuestions.map((item, itemIndex) => itemIndex === index ? { ...item, options: event.target.value } : item) })} /></label><div style={{ display: 'grid', gridTemplateColumns: '1fr 110px', gap: 12, marginTop: 8 }}><label className="label">Correct label<input className="input" placeholder="A" value={subQuestion.correctAnswers} onChange={(event) => setForm({ ...form, subQuestions: form.subQuestions.map((item, itemIndex) => itemIndex === index ? { ...item, correctAnswers: event.target.value } : item) })} /></label><label className="label">Points<input className="input" type="number" min="1" value={subQuestion.points} onChange={(event) => setForm({ ...form, subQuestions: form.subQuestions.map((item, itemIndex) => itemIndex === index ? { ...item, points: event.target.value } : item) })} /></label></div></div>)}</div>}
