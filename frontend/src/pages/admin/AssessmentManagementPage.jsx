...
const INITIAL = { title: '', description: '', assessmentType: 'MCQ', durationMinutes: 60, passingScore: 70, instructions: '', startTime: '', endTime: '', shuffleQuestions: false, shuffleOptions: false, resultsVisible: false, feedbackVisible: true };
...
export default function AssessmentManagementPage() {
  ...
  const questionCategories = useMemo(() => [...new Set(questions.map((question) => question.category).filter(Boolean))].sort(), [questions]);
  const filteredQuestions = useMemo(() => {
    const search = questionSearch.trim().toLowerCase();
    return questions.filter((question) => {
      const type = question.questionType ?? question.type ?? '';
      const searchable = `${question.title ?? ''} ${question.content ?? ''} ${question.category ?? ''} ${(question.tags ?? []).join(' ')}`.toLowerCase();
      return (!search || searchable.includes(search))
        ...
        && (questionCategoryFilter === 'ALL' || question.category === questionCategoryFilter);
    });
  }, [questions, questionSearch, questionTypeFilter, questionDifficultyFilter, questionCategoryFilter]);
  const clearQuestionFilters = () => { setQuestionSearch(''); setQuestionTypeFilter('ALL'); setQuestionDifficultyFilter('ALL'); setQuestionCategoryFilter('ALL'); };
  ...
  const sendFinalScores = async () => { if (!resultSettings) return; try { const finalSettings = { ...resultSettings, resultsVisible: true }; const { data } = await assessmentAPI.update(finalSettings.id, finalSettings); setAssessments((items) => items.map((item) => item.id === data.id ? data : item)); setResultSettings(null); try { await assessmentAPI.sendFinalScores(finalSettings.id); toast.success('Final scores sent to assigned candidates.'); } catch { toast.success('Final scores are now visible to candidates.'); } } catch (error) { toast.error(error.response?.data?.message || 'Unable to publish final scores.'); } };
  const removeAssessment = async (assessment) => {
    if (!window.confirm(`Delete “${assessment.title}”? This also removes its sections and assignments. This cannot be undone.`)) return;
    try {
      await assessmentAPI.delete(assessment.id);
      setAssessments((items) => items.filter((item) => item.id !== assessment.id));
      toast.success('Assessment deleted.');
    } catch (error) {
      toast.error(error.response?.data?.message || 'Unable to delete the assessment.');
    }
  };
  ...
  return <div className="container fade-in">
    ...
    <div className="card">{assessments.length === 0 ? <div className="empty-state"><div className="empty-title">No assessments yet</div><div className="empty-subtitle">Create one from your question bank.</div></div> : <div className="question-stack">{assessments.map((assessment) => <div className="choice-option" key={assessment.id} style={{ justifyContent: 'space-between' }}><div><strong>{assessment.title}</strong><div className="text-secondary" style={{ fontSize: 13, marginTop: 4 }}>{assessment.assessmentType} · {assessment.durationMinutes} minutes · {assessment.isPublished ? 'Published' : 'Draft'}</div></div><div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', justifyContent: 'flex-end' }}><div style={{ position: 'relative' }}><button className="btn btn-secondary btn-sm" onClick={() => setOpenActionMenuId((current) => current === assessment.id ? null : assessment.id)}>Manage ▾</button>{openActionMenuId === assessment.id && <div className="assessment-action-menu"><div className="assessment-action-label">Delivery</div>{!assessment.isPublished && <button onClick={() => { setOpenActionMenuId(null); publish(assessment); }}>Publish assessment</button>}<button onClick={() => { setOpenActionMenuId(null); setAssignment(assessment); }}>Assign candidates</button><div className="assessment-action-label">Review</div><button onClick={() => { setOpenActionMenuId(null); viewTracking(assessment); }}>Assignment tracking</button><button onClick={() => { setOpenActionMenuId(null); setResultSettings({ ...assessment, resultsVisible: assessment.resultsVisible !== false, feedbackVisible: assessment.feedbackVisible !== false }); }}>Candidate results</button></div>}</div><button className="btn btn-ghost btn-sm" style={{ color: 'var(--red)' }} onClick={() => removeAssessment(assessment)}>Delete</button></div></div>)}</div>}</div>
    ...
    {resultSettings && <div className="modal-backdrop" role="dialog" aria-modal="true"><div className="card" style={{ width: 'min(460px, 94vw)', padding: 24 }}><div className="card-header"><div className="card-title">Candidate results · {resultSettings.title}</div><button className="btn btn-ghost btn-sm" onClick={() => setResultSettings(null)}>Close</button></div><p className="text-secondary" style={{ marginTop: 10, fontSize: 13 }}>Choose what candidates can see, then send the finalized score when grading is complete.</p><label style={{ display: 'block', marginTop: 12 }}><input type="checkbox" checked={resultSettings.resultsVisible} onChange={(event) => setResultSettings({ ...resultSettings, resultsVisible: event.target.checked })} /> Show candidates their score and section results</label><label style={{ display: 'block', marginTop: 12, opacity: resultSettings.resultsVisible ? 1 : 0.55 }}><input type="checkbox" disabled={!resultSettings.resultsVisible} checked={resultSettings.feedbackVisible} onChange={(event) => setResultSettings({ ...resultSettings, feedbackVisible: event.target.checked })} /> Show evaluator feedback</label><div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, flexWrap: 'wrap', marginTop: 20 }}><button className="btn btn-secondary" onClick={() => setResultSettings(null)}>Cancel</button><button className="btn btn-secondary" onClick={saveResultSettings}>Save settings</button><button className="btn btn-primary" onClick={sendFinalScores}>Send final scores</button></div></div></div>}
    {open && <div className="modal-backdrop" role="dialog" aria-modal="true"><form className="card" onSubmit={create} style={{ width: 'min(820px, 94vw)', maxHeight: '90vh', overflowY: 'auto', padding: 24 }}><div className="card-header"><div className="card-title">Create assessment</div><button type="button" className="btn btn-ghost btn-sm" onClick={() => setOpen(false)}>Close</button></div><div style={{ display: 'grid', gridTemplateColumns: '1fr 180px 130px', gap: 12 }}><label className="label">Title<input className="input" required value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} /></label><label className="label">Type<select className="select" value={form.assessmentType} onChange={(e) => setForm({ ...form, assessmentType: e.target.value })}><option value="MCQ">MCQ</option><option value="CODING">Coding</option><option value="WRITTEN">Written</option><option value="MIXED">Mixed</option></select></label><label className="label">Minutes<input className="input" required type="number" min="1" value={form.durationMinutes} onChange={(e) => setForm({ ...form, durationMinutes: e.target.value })} /></label></div><div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginTop: 14 }}><label className="label">Opens at<input className="input" type="datetime-local" value={form.startTime} onChange={(e) => setForm({ ...form, startTime: e.target.value })} /></label><label className="label">Closes at<input className="input" type="datetime-local" value={form.endTime} onChange={(e) => setForm({ ...form, endTime: e.target.value })} /></label></div><label className="label" style={{ display: 'block', marginTop: 14 }}>Description<textarea className="textarea" value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} /></label><label className="label" style={{ display: 'block', marginTop: 14 }}>Instructions<textarea className="textarea" value={form.instructions} onChange={(e) => setForm({ ...form, instructions: e.target.value })} /></label><label className="label" style={{ display: 'block', marginTop: 14, maxWidth: 160 }}>Passing score<input className="input" type="number" min="0" max="100" value={form.passingScore} onChange={(e) => setForm({ ...form, passingScore: e.target.value })} /></label><div style={{ display: 'flex', gap: 18, flexWrap: 'wrap', marginTop: 14 }}><label><input type="checkbox" checked={form.shuffleQuestions} onChange={(e) => setForm({ ...form, shuffleQuestions: e.target.checked })} /> Shuffle questions</label><label><input type="checkbox" checked={form.shuffleOptions} onChange={(e) => setForm({ ...form, shuffleOptions: e.target.checked })} /> Shuffle options</label><label><input type="checkbox" checked={form.resultsVisible} onChange={(e) => setForm({ ...form, resultsVisible: e.target.checked })} /> Show candidates their result</label><label><input type="checkbox" checked={form.feedbackVisible} disabled={!form.resultsVisible} onChange={(e) => setForm({ ...form, feedbackVisible: e.target.checked })} /> Show evaluator feedback</label></div><div style={{ marginTop: 18 }}><div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}><strong>Sections</strong><button type="button" className="btn btn-secondary btn-sm" onClick={() => setSections((items) => [...items, newSection(items.length + 1)])}>+ Add section</button></div><div className="assessment-question-filters"><div><strong>Find questions</strong><small>{filteredQuestions.length} of {questions.length} shown</small></div><input className="input" value={questionSearch} onChange={(event) => setQuestionSearch(event.target.value)} placeholder="Search title, content, tags..." /><select className="select" value={questionTypeFilter} onChange={(event) => setQuestionTypeFilter(event.target.value)}><option value="ALL">All types</option><option value="MCQ_SINGLE">Multiple choice</option><option value="MCQ_MULTI">Multiple select</option><option value="WRITTEN">Written</option><option value="CODING">Algorithmic coding</option><option value="DEBUGGING">Debugging</option><option value="SQL">SQL</option><option value="API_IMPLEMENTATION">API implementation</option><option value="READING_COMPREHENSION">Reading comprehension</option><option value="FILE_UPLOAD">File upload</option></select><select className="select" value={questionDifficultyFilter} onChange={(event) => setQuestionDifficultyFilter(event.target.value)}><option value="ALL">All difficulties</option><option value="EASY">Easy</option><option value="MEDIUM">Medium</option><option value="HARD">Hard</option></select><select className="select" value={questionCategoryFilter} onChange={(event) => setQuestionCategoryFilter(event.target.value)}><option value="ALL">All categories</option>{questionCategories.map((category) => <option key={category} value={category}>{category}</option>)}</select><button type="button" className="btn btn-ghost btn-sm" onClick={clearQuestionFilters}>Clear</button></div>{sections.map((section, index) => <div className="card" key={section.id} style={{ marginTop: 12, padding: 14 }}><div style={{ display: 'flex', gap: 10, alignItems: 'end' }}><label className="label" style={{ flex: 1 }}>Section {index + 1} name<input className="input" value={section.title} onChange={(e) => updateSection(section.id, { title: e.target.value })} /></label>{sections.length > 1 && <button type="button" className="btn btn-ghost btn-sm" onClick={() => setSections((items) => items.filter((item) => item.id !== section.id))}>Remove</button>}</div><div className="text-secondary" style={{ fontSize: 13, marginTop: 8 }}>Select questions for this section ({section.questionIds.length})</div><div className="question-stack" style={{ marginTop: 10, maxHeight: 280, overflowY: 'auto' }}>{filteredQuestions.length === 0 ? <div className="empty-state" style={{ padding: 24 }}><div className="empty-subtitle">No questions match these filters.</div></div> : filteredQuestions.map((question) => <label className="choice-option" key={question.id} style={{ display: 'flex', gap: 10, cursor: 'pointer' }}><input type="checkbox" checked={section.questionIds.includes(question.id)} onChange={() => toggleQuestion(section.id, question.id)} /><span><strong>{question.title}</strong><small className="text-secondary" style={{ display: 'block' }}>{question.questionType ?? question.type} · {question.difficulty} · {question.points ?? 1} points{question.category ? ` · ${question.category}` : ''}</small></span></label>)}</div></div>)}</div><div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, marginTop: 20 }}><button type="button" className="btn btn-secondary" onClick={() => setOpen(false)}>Cancel</button><button className="btn btn-primary" disabled={saving}>{saving ? 'Creating...' : 'Create draft'}</button></div></form></div>}
  </div>;
}
