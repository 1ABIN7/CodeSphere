...
const EMPTY = { summary: { averageScore: 0, completionRate: 0, passRate: 0 }, assessments: [] };
...
export default function ReportsDashboard() {
  ...
  const difficultyData = useMemo(() => ['EASY', 'MEDIUM', 'HARD'].map((difficulty) => {
    ...
    return { difficulty: difficulty[0] + difficulty.slice(1).toLowerCase(), success: items.length ? items.reduce((sum, item) => sum + item.successRate, 0) / items.length : 0, questions: items.length };
  }), [questionMetrics]);
  ...
  return <div className="container fade-in">
    ...
    <div className="dashboard-grid">{[{ label: 'Average score', value: percent(summary.averageScore) }, { label: 'Completion rate', value: percent(summary.completionRate) }, { label: 'Pass rate', value: percent(summary.passRate) }, { label: 'Top performer', value: topPerformer ? topPerformer.name : '—', small: true }].map((metric) => <div className="card stat-card" key={metric.label}><div className="stat-card-value" style={metric.small ? { fontSize: 19 } : {}}>{metric.value}</div><div className="stat-card-label">{metric.label}</div></div>)}</div>
    {!loading && <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24, marginTop: 24 }}>
      ...
      <ChartCard title="Difficulty analysis" subtitle="Average success rate for attempted questions"><ResponsiveContainer width="100%" height={260}><BarChart data={difficultyData}><CartesianGrid strokeDasharray="3 3" opacity={0.2} /><XAxis dataKey="difficulty" /><YAxis domain={[0, 100]} /><Tooltip formatter={(value) => percent(value)} /><Bar dataKey="success" radius={[5, 5, 0, 0]} fill="#22c55e" /></BarChart></ResponsiveContainer></ChartCard>
      <ChartCard title="Question effectiveness" subtitle="Most-attempted questions by success rate"><ResponsiveContainer width="100%" height={260}><BarChart data={[...questionMetrics].sort((a, b) => b.attempts - a.attempts).slice(0, 8)}><CartesianGrid strokeDasharray="3 3" opacity={0.2} /><XAxis dataKey="title" tick={{ fontSize: 11 }} interval={0} angle={-18} textAnchor="end" height={60} /><YAxis domain={[0, 100]} /><Tooltip formatter={(value) => percent(value)} /><Bar dataKey="successRate" radius={[5, 5, 0, 0]} fill="#f59e0b" /></BarChart></ResponsiveContainer></ChartCard>
    </div>}
    <ReportTable title="Assessment details" headers={['Assessment', 'Candidates', 'Completion', 'Average score', 'Pass rate']} empty="No assessment results are available yet.">{(report.assessments ?? []).map((item) => <tr key={item.id}><td>{item.title}</td><td>{item.candidates ?? 0}</td><td>{percent(item.completionRate)}</td><td>{percent(item.averageScore)}</td><td>{percent(item.passRate)}</td></tr>)}</ReportTable>
    ...
    <ReportTable title="Question effectiveness" headers={['Question', 'Attempts', 'Success', 'Avg. score', 'Avg. time']} empty="Question metrics appear after candidates submit answers.">{questionMetrics.map((item) => <tr key={item.id}><td><strong>{item.title}</strong><div className="text-secondary" style={{ fontSize: 12 }}>{item.questionType} · {item.difficulty || 'Unclassified'}</div></td><td>{item.attempts}</td><td>{percent(item.successRate)}</td><td>{percent(item.averageScore)}</td><td>{item.averageSecondsSpent}s</td></tr>)}</ReportTable>
  </div>;
}
